...
class Config {
    ...
    public Config() {
        try {
            InputStream is = new FileInputStream("T:\\CNProject2\\src\\com\\company\\Common.cfg");
...
class Peer {
    ...
    public void retrieve_Peer_Details() throws FileNotFoundException {
        ...
        BufferedReader b_R = new BufferedReader(new FileReader("D:\\CNProject2\\src\\com\\company\\PeerInfo.cfg"));
        String line = null;
        try {
...
public class peerProcess {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if(peer.initFileExistsG_t())
            SptFile();
        Srvercls svr = new Srvercls(peer.portnum_gt(), peer);
        peer.Srv_Setter(svr);
        svr.start();

        for(peerData peerInfoServer : peer.retrieve_P2Conn()) {
            ...
            try {
                ...
                HandShake handshakeObj = new HandShake(peer.getPid());
                ...
                ConnectionDetails contInfo = new ConnectionDetails(peerInfoServer.getPid(), reqSocket, out, inreslt, peer, new ConcurrentLinkedQueue<Object>());
                //Add them to peer
    ...
    //Determining new set of neighbours to choke from previous list of preferred neighbours
        ...
        Map<String, ConnectionDetails> prConnect = peer. retrieveP2PCon();
        Set<String> prfNbr = peer. retrievePrefNeighbours();

        for(String peerID : prfNbr) {
            ConnectionDetails contInfo = prConnect.get(peerID);
            contInfo.send(new ActualMessage(0, null));
    ...
    public static void getNbrMsg() throws IOException {
        ...
        PriorityQueue<Downloasdrate_n> priorityQ = new PriorityQueue<Downloasdrate_n>((a, b) -> b.download_spd - a.download_spd);

        for(String key : download_spd.keySet()) {
            priorityQ.add(new Downloasdrate_n(key, download_spd.get(key)));
        ...
        while(prefNbrNum > 0 && priorityQ.size() != 0) {
            Downloasdrate_n node = priorityQ.poll();
    ...
    public static void msgUnchoked() throws IOException {
        ...
        for(String pID : peer.retrievePrefNeighbours()) {
            ConnectionDetails contInfo = prConnect.get(pID);
            contInfo.send(new ActualMessage(1, null));
        }
...
//            accepted.remove(pID);
    ...
    public static void SptFile() throws IOException {
        ...
        File fls = new File("T:\\CNProject2\\src\\com\\company\\thefile");
    ...
    public static void pecSelectAlgo() {
        ...
        PriorityBlockingQueue<node_slct> priorityQ = new PriorityBlockingQueue<node_slct>(10, (a, b) -> b.pty - a.pty);

        List<node_slct> lst = new ArrayList<>();

        for(int i = 0; i < Config.totPieces; i++) {
            if(i >= min && i <= max) {
                lst.add(new node_slct(2, i));
            }
            else {
                lst.add(new node_slct(1, i));
            }
        }
    ...
    public static boolean completedBool() throws InterruptedException, IOException {
        List<peerData> totalPrs = peer.retrieve_Peers();
        int N = 0;
        for(peerData Pr1 : totalPrs) {
            String btFd;
    ...
    public static void exitAll() throws IOException, InterruptedException {
        ...
        ConcurrentMap<String, ConnectionDetails> connts = peer.retrieveP2PCon();

        for(String peer : connts.keySet()) {
            ConnectionDetails conn2 = connts.get(peer);
